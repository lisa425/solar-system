...
export default class Sun {
  ...
  createSun() {
    const material = new MeshStandardMaterial({
      map: this.loader.load(this.sunTexture),
    })
  ...
  createCorona() {
    ...
    const update = (t) => {
      for (let i = 0; i < len; i += 1) {
        ...
        let ns = coronaNoise.noise(v3.x + Math.cos(t), v3.y + Math.sin(t), v3.z + t)
        v3.copy(p)
          .setLength(5)
  ...
  createRim() {
    const uniforms = {
      color1: { value: new Color(0xfff7cc) },
      color2: { value: new Color(0x000000) },
      fresnelBias: { value: 0.35 },
